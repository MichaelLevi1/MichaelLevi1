    ...
    def step(self):
        ...
        if (self.show_figure):
            ...
            for i in range(self.number_of_robots):
                ...
                def position(Array, Centre, theta):
                    ...
                    for i in range(0, length):
                        rotated_matrix[i,:] = np.dot(rot_mat, rotated_matrix[i,:])
                    rotated_matrix = rotated_matrix + Centre
                ...
                if self.Aircraft_Type == 1: # X8 Polygon
                    ...
                    Aircraft_Corrected = []
                    Aircraft_Corrected = position(self.Aircraft_Coord, self.poses[:2, i], self.poses[2, i] + math.pi / 4)
                    p = patches.Polygon(Aircraft_Corrected, True, facecolor='#2ECC71', edgecolor='k')
                    ...
                    Prop_Centre = np.empty(shape=(4, 2))
                    for j in range(0,4):
                        Prop_Centre[j][0] = Aircraft_Corrected[j*(self.N+3)-2][0]
                        Prop_Centre[j][1] = Aircraft_Corrected[j*(self.N+3)-2][1]
                    prop_1 = patches.Ellipse(Prop_Centre[0][:], 0.01, 0.075, (t%1)*360*2, facecolor='#4A4A4A')
                    self.axes.add_patch(prop_1)
                    prop_2 = patches.Ellipse(Prop_Centre[1][:], 0.01, 0.075, -(t%1)*360*2, facecolor='#4A4A4A')
                    self.axes.add_patch(prop_2)
                    prop_3 = patches.Ellipse(Prop_Centre[2][:], 0.01, 0.075, (t%1)*360*2, facecolor='#4A4A4A')
                    self.axes.add_patch(prop_3)
                    prop_4 = patches.Ellipse(Prop_Centre[3][:], 0.01, 0.075, -(t%1)*360*2, facecolor='#4A4A4A')
                    self.axes.add_patch(prop_4)

                elif self.Aircraft_Type == 3: # X8 Corrected
                ...
                elif self.Aircraft_Type == 4: # S500 Quadcopter Corrected
            ...
            # Due to use of patches.Polygon, the display did not work with the usual method
            # This section is for only the polygon versions of the X8 and S500

            if self.Aircraft_Type == 1 or self.Aircraft_Type == 2:
                plt.cla()
                # Draw arena
